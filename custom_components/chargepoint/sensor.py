...
CHARGER_SENSORS = [
    ...
    ChargePointSensorEntityDescription(key="session_charging_time", name_suffix="Session Charging Time", icon="mdi:timer-outline", value=lambda e: _format_duration(e.session.charging_time) if e.session else "00:00:00"),
    ChargePointSensorEntityDescription(key="status", name_suffix="Status", value=lambda e: str(e.charger_status.charging_status).replace('_', ' ').title() if e.charger_status.plugged_in else "Not Connected"),
    ChargePointSensorEntityDescription(key="charger_state", name_suffix="Charger State", value=lambda e: str(e.session.charging_state).replace('_', ' ').title() if e.session and e.session.charging_state else "Not Charging"),
    ChargePointSensorEntityDescription(key="account_balance", name_suffix="Account Balance", device_class=SensorDeviceClass.MONETARY, state_class=SensorStateClass.TOTAL, native_unit_of_measurement="CAD", icon="mdi:wallet", value=lambda e: _safe_float(getattr(e.coordinator.data[ACCT_INFO], 'balance', getattr(e.coordinator.data[ACCT_INFO], 'credit_balance', 0.0))) if e.coordinator.data.get(ACCT_INFO) else 0.0),
]
